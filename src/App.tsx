...
export default function App() {
  ...
  return <div className="app-shell"><Sidebar active={active} onChange={setActive} mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} providers={providers} conversations={conversations} selectedId={selectedId} onSelect={selectChat} onNew={createChat} onDelete={deleteChat} /><section className="workspace"><Topbar title={active} connected={providers.length > 0} onMenu={() => setMobileOpen(true)} onSearch={() => setSearchOpen(true)} onSettings={() => setSettingsOpen(true)} /><div className="workspace-content">{active === "Chat" && <ChatView conversation={selected} providers={providers} onUpdate={updateMessages} onCompare={startCompare} />}{active === "Provider vault" && <ProviderVault providers={providers} onRefresh={refreshProviders} />}{active === "Compare models" && <CompareModels providers={providers} initialPrompt={comparePrompt} onUse={useComparison} />}{active === "Usage & cost" && <UsageView usage={usage} onRefresh={refreshUsage} />}{active === "Coding agent" && <CodingAgent providers={providers} />}</div></section>
